...
def configure_ngrok():
    ngrok_token = "2qepkKlui33fZ6ilRtrdt6SNIP1_6A7tHC4hiVZ6KdPjUdKYp"
    try:
...
def update_soul_txt(public_url):
    with open("soul10.txt", "w") as file:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("soul10.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
